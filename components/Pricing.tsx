...
function PlanCard({ p, annual }: { p: typeof plans[0]; annual: boolean }) {
  ...
  return (
    <motion.div variants={item}
      className={`rounded-2xl p-6 sm:p-8 flex flex-col relative bg-white border-2 transition-all ${
        p.popular ? "border-[#51197e] shadow-xl shadow-[#51197e]/10" : "border-gray-100 shadow-sm hover:shadow-lg"
      }`}
    >
      {p.popular && (
      ...
      <h3 className="text-2xl font-bold mb-1 text-gray-900">{p.name}</h3>
      <p className="text-sm text-gray-500 mb-6">{p.desc}</p>

      <div className="mb-2">
        {p.monthly ? (
          <>
            <span className="text-4xl sm:text-5xl font-extrabold text-gray-900" style={{ fontFamily: "'Neue Machina', sans-serif" }}>
              {annual ? p.yearly : p.monthly}€
            </span>
            <span className="text-gray-500 text-sm font-medium">/mois TTC</span>
          </>
        ) : (
          <span className="text-3xl font-extrabold text-gray-900" style={{ fontFamily: "'Neue Machina', sans-serif" }}>
            Sur devis
          </span>
        )}
      </div>
      ...
      <ul className="flex flex-col gap-3 mb-4 flex-1">
        {visibleFeatures.map((f) => (
          <li key={f.text} className="flex items-start gap-2.5 text-sm">
            <span className="inline-flex items-center justify-center w-5 h-5 rounded-full bg-green-100 text-green-600 text-xs font-bold flex-shrink-0 mt-0.5">✓</span>
            <div>
              <span className="text-gray-900 font-medium">{f.text}</span>
              {f.desc && <p className="text-[11px] text-gray-400 mt-0.5 leading-snug">{f.desc}</p>}
            </div>
          </li>
        ))}
      </ul>
      ...
      {hasMore && (
        <button
          ...
          className="text-[#51197e] text-xs font-semibold mb-6 flex items-center gap-1 hover:underline outline-none focus:outline-none"
        >
      ...
      <a href="#"
        className={`text-center py-3.5 rounded-full font-bold text-sm transition-all group flex items-center justify-center gap-2 ${
          p.popular
            ...
            : "border-2 border-[#51197e] text-[#51197e] hover:bg-[#51197e] hover:text-white"
        }`}
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="flex items-center justify-center gap-4 mb-14">
          ...
          <button onClick={() => setAnnual(!annual)}
            ...
            style={{ background: annual ? "#51197e" : "#e5e7eb" }}>
          ...
          <span className={`text-sm font-semibold ${annual ? "text-gray-900" : "text-gray-400"}`}>
            Annuel <span className="text-[#51197e] text-xs font-bold ml-1 px-2 py-0.5 rounded-full" style={{ background: "#EDE5F7" }}>-20%</span>
          </span>
        </div>
        ...
        >
          <div className="flex flex-col sm:flex-row items-center gap-4 p-5 sm:p-6 rounded-2xl border-2 border-dashed border-[#51197e]/30 bg-[#F9F7FC]">
            <div className="w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: "#EDE5F7" }}>
              <span className="text-2xl">🎁</span>
            </div>
            <div className="flex-1 text-center sm:text-left">
              <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2 mb-1">
                <h4 className="font-bold text-gray-900">Pack Fidélité</h4>
                <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold text-[#51197e]" style={{ background: "#EDE5F7" }}>Option</span>
              </div>
              <p className="text-xs text-gray-500">Roulette de récompenses et bons de réduction pour fidéliser vos clients.</p>
            </div>
            <div className="flex-shrink-0 text-center">
              <span className="text-2xl font-extrabold text-gray-900" style={{ fontFamily: "'Neue Machina', sans-serif" }}>+10€</span>
              <span className="text-gray-500 text-xs font-medium">/mois</span>
            </div>
          </div>
        </motion.div>
        ...
        >
          {[
            ...
            <div key={g.title} className="text-center p-5 rounded-xl border border-gray-100 bg-gray-50">
              <span className="text-2xl mb-2 block">{g.icon}</span>
              <h4 className="font-bold text-sm text-gray-900 mb-1">{g.title}</h4>
